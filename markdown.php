...
#
# Modifications towards php5 OO by cfv1984
#
# PHP Markdown
...
class Markdown_Parser
{
    ...
    public function stripLinkDefinitions($text)
    {
        ...
        # hash references.
        #
        $less_than_tab = Markdown_Parser::MARKDOWN_TAB_WIDTH - 1;
    ...
    public function doHorizontalRules($text)
    {
        ...
        return preg_replace(
            ...
            "\n" . $this->hashBlock("<hr" . Markdown_Parser::MARKDOWN_EMPTY_ELEMENT_SUFFIX) . "\n",
            $text);
    }
    ...
    {
        return $this->hashPart("<br" . Markdown_Parser::MARKDOWN_EMPTY_ELEMENT_SUFFIX . "\n");
    }
    ...
    private function _doImages_reference_callback($matches)
    {
        ...
        if (isset($this->urls[$link_id])) {
            ...
            $result .= Markdown_Parser::MARKDOWN_EMPTY_ELEMENT_SUFFIX;
            $result = $this->hashPart($result);
        }
        else {
    ...
    public function doCodeBlocks($text)
    {
        ...
        $text = preg_replace_callback('{
				...
				(	            # $1 = the code block -- one or more lines, starting with a space/tab
				  (?>
					[ ]{' . Markdown_Parser::MARKDOWN_TAB_WIDTH . '}  # Lines must start with a tab or a tab-width of spaces
					.*\n+
				  )+
				)
				((?=^[ ]{0,' . Markdown_Parser::MARKDOWN_TAB_WIDTH . '}\S)|\Z)	# Lookahead for non-space at line-start, or end of doc
			}xm',
    ...
    public function outdent($text)
    {
        ...
        #
        return preg_replace('/^(\t|[ ]{1,' . Markdown_Parser::MARKDOWN_TAB_WIDTH . '})/m', '', $text);
    }
    ...
    private function _detab_callback($matches)
    {
        ...
        foreach ($blocks as $block) {
            ...
            $amount = Markdown_Parser::MARKDOWN_TAB_WIDTH;
            $strlen($line, 'UTF-8') % Markdown_Parser::MARKDOWN_TAB_WIDTH;
